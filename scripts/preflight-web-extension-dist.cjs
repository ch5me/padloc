...
failIf("background.js", background, [
    { pattern: /\bXMLHttpRequest\b/, reason: "service worker bundle must not include XMLHttpRequest" },
    {
        ...
        reason: "service worker bundle must not include page history router calls",
    },
    ...
    {
        pattern: /onMessage\.addListener\s*\(\s*async\b/,
        reason: "service worker message listeners must not be async catch-alls",
    },
    {
        pattern: /name:\s*["']Private Key["']|["']Private Key["']\s*,\s*type:/,
        reason: "service worker bundle must not store passkey private keys in vault fields",
    },
]);
