...
mdLines.push("");
mdLines.push("- **Success (200)**: `Content-Type: application/json; charset=utf-8`. Body is `marshal(Response.toRaw())`.");
mdLines.push("- **Bad Request (400)**: Returned on any exception during request processing, including parse/decode failures.");
...
mdLines.push("## Notes");
mdLines.push("");
mdLines.push("- `String` param type maps to `string` in inventory; decorator converts `String` → `undefined` in handlerDefinitions.");
...
evidenceLines.push("");
evidenceLines.push("Auth flow documented: startAuthRequest → completeAuthRequest → startCreateSession → completeCreateSession");
evidenceLines.push("");
evidenceLines.push("HTTP error shape captured from packages/server/src/transport/http.ts:38-50 (config) + 78-106 (POST handler).");
evidenceLines.push("");
...
const learningsPath = join(
  ROOT,
  ".sisyphus/notepads/padloc-cloudflare-native-backend/learnings.md"
);
