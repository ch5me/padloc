...
const forbiddenBackgroundSource = [
    ...
    {
        pattern: /(^|[^.\w$])history\s*\.(?:state|replaceState|pushState|go)\b/m,
        reason: "service worker graph must not call page history",
    },
...
function requireAuthRestart(methodName, block, errorCodes) {
    ...
    for (const code of errorCodes) {
        ...
        if (!pattern.test(block)) {
            failures.push(
                `packages/app/src/elements/login-signup.ts: ${methodName} must restart email verification on ${code}`
            );
        }
    }
}
...
function checkBackgroundSource() {
    ...
    if (!backgroundSource.includes("dedupeMatchedItems")) {
        failures.push(
            "packages/extension/src/background.ts: context menu matched items must be deduplicated before create"
        );
    }

    if (!backgroundSource.includes("createContextMenuOnce")) {
        failures.push(
            "packages/extension/src/background.ts: context menu item creation must be idempotent within a rebuild"
        );
    }

    if (
        !backgroundSource.includes("duplicate id") ||
        !backgroundSource.includes("browser.contextMenus.remove(createProperties.id)")
    ) {
        failures.push(
            "packages/extension/src/background.ts: context menu create must remove stale duplicate ids before retry"
        );
    }
}
...
function checkExtensionUiSource() {
    for (const file of [...walkFiles("packages/app/src"), ...walkFiles("packages/extension/src")]) {
        const source = fs.readFileSync(file, "utf8");
        const relativePath = relativeFromRoot(file);
    ...
    if (/reset\s*\(\)\s*\{[\s\S]*?this\.requestUpdate\s*\(/.test(startFormSource)) {
        failures.push(
            "packages/app/src/elements/start-form.ts: reset() must not schedule another update from updated()"
        );
    }
    ...
    if (/addEventListener\s*\(\s*["']visibilitychange["']\s*,\s*\(\)\s*=>/.test(unlockSource)) {
        failures.push(
            "packages/app/src/elements/unlock.ts: visibilitychange listener must be removable, not anonymous"
        );
    }
    ...
    if (
        ...
        failures.push(
            "packages/extension/scripts/agentic-extension-cdp.mjs: clear-storage must fail when signer IndexedDB deletion is blocked"
        );
    }
    if (
        cdpHelperSource.includes('indexedDB.deleteDatabase("padloc-agentic-passkey-signers")') &&
        ...
        failures.push(
            "packages/extension/scripts/agentic-extension-cdp.mjs: clear-storage must verify signer IndexedDB is empty"
        );
    }
    if (
        !cdpHelperSource.includes("pingExtensionRuntime") ||
        !cdpHelperSource.includes("readDistAsset") ||
        !cdpHelperSource.includes("extension runtime ping")
    ) {
        failures.push(
