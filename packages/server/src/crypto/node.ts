...
export class NodeCryptoProvider implements CryptoProvider {
    ...
    private async _signHMAC(key: HMACKey, data: Uint8Array, params: HMACParams): Promise<Uint8Array> {
        ...
        // Hash keys longer than 32 bytes down to 32 bytes for cross-platform compatibility
        // (Web Crypto rejects HMAC keys > hash output size).
        const effectiveKey = key.length > 32 ? createHash("sha256").update(key).digest() : Buffer.from(key);
        const hmac = createHmac(hash, effectiveKey);
