...
const port = Number(process.env.ORG_SEAT_QUOTA_TEST_PORT || 18792);
const packageRoot = new URL("..", import.meta.url);
const persistDir = mkdtempSync(join(tmpdir(), "padloc-org-seat-quota-"));
const wranglerArgs = [
    "dev",
    "test/org-seat-quota-e2e.worker.ts",
    "--local",
    "--persist-to",
    persistDir,
    "--env=dev",
...
function requestTests() {
    return new Promise((resolve, reject) => {
        const req = http.get(`http://127.0.0.1:${port}/org-seat-quota-tests`, (res) => {
            let body = "";
        ...
        req.setTimeout(30000, () => {
            req.destroy(new Error("Timed out waiting for org seat quota test response"));
        });
    });
}
