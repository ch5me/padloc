...
const port = Number(process.env.ORG_ISOLATION_TEST_PORT || 18791);
const packageRoot = new URL("..", import.meta.url);
const persistDir = mkdtempSync(join(tmpdir(), "padloc-org-isolation-"));
const wranglerArgs = [
    "dev",
    "test/multi-org-isolation-e2e.worker.ts",
    "--local",
    "--persist-to",
    persistDir,
    "--env=dev",
...
function requestTests() {
    return new Promise((resolve, reject) => {
        const req = http.get(`http://127.0.0.1:${port}/multi-org-isolation-tests`, (res) => {
            let body = "";
        ...
        req.setTimeout(30000, () => {
            req.destroy(new Error("Timed out waiting for multi-org isolation test response"));
        });
    });
}
