...
const port = Number(process.env.FIREFLY_SEAT_SYNC_TEST_PORT || 18793);
const packageRoot = new URL("..", import.meta.url);
const persistDir = mkdtempSync(join(tmpdir(), "padloc-firefly-seat-sync-"));
const wranglerArgs = [
    "dev",
    "test/firefly-seat-sync-e2e.worker.ts",
    "--local",
    "--persist-to",
    persistDir,
    "--env=dev",
...
async function startChild() {
    ...
    execFileSync("wrangler", ["d1", "migrations", "apply", "DB", "--local", "--env=dev", "--persist-to", persistDir], {
        cwd: packageRoot,
        stdio: "ignore",
    });
...
function requestTests() {
    return new Promise((resolve, reject) => {
        const req = http.get(`http://127.0.0.1:${port}/firefly-seat-sync-tests`, (res) => {
            let body = "";
        ...
        req.setTimeout(30000, () => {
            req.destroy(new Error("Timed out waiting for firefly seat sync test response"));
        });
    });
}
