...
- The extension RP lane refuses an already-owned Worker port, bounds startup
  and shutdown, escalates only its own child process, and verifies port release.
- Each run uses a unique local canary identity, removes only that identity, and
  leaves no Playwright report or extension artifact that was not present before
  the run.
...
- Native polling and hardware approval are bounded. Provider evidence stays in
  memory and is reduced to accepted/verified counts plus a redacted fingerprint;
  raw unified logs are never uploaded.
...
bypasses it in Release nor counts as biometric, Apple Watch, device-password,
or system-sheet evidence.
...
Safari/provider restart, a persisted second assertion, and redacted CH5
provider provenance. This proves the attended hardware boundary for the current
machine; it does not change Gate 7 or turn synthetic injection into biometric
evidence.
