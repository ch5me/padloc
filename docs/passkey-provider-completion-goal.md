...
- The controlled `https://passkey-test.ch5.me` registration and authentication ceremony passed through the real extension, including popup approval, RP-ID/origin validation, signature verification in the RP page, strictly acknowledged encrypted-vault persistence, synchronized-passkey backup flags, the required zero-counter policy, and reload.
